...
public class FastDFSManager implements Serializable {
	...
	static {
		try {
	           // String classPath = new File(FastDFSManager.class.getResource("/").getFile()).getCanonicalPath();
	           // String cfgPath = classPath + File.separator + CLIENT_CONFIG_FILE;
    ...
    public static String url(String group, String path) throws Exception {
        ...
        String url = PROTOCOL + fileInfo.getSourceIpAddr()
                + SEPARATOR
                + group
                + SEPARATOR
                + path;
